...
import (
	...
	"gitlab.lainuoniao.cn/eden-quan/protoc-gen-go-errors-fx/errors"
)
...
func generateFile(gen *protogen.Plugin, file *protogen.File) *protogen.GeneratedFile {
	...
	g.QualifiedGoIdent(stringsPackage.Ident(""))
...
func genErrorsReason(_ *protogen.Plugin, _ *protogen.File, g *protogen.GeneratedFile, enum *protogen.Enum) bool {
	...
	var ew errorWrapper
	ew.ErrorEnumName = string(enum.Desc.Name())
	...
	for _, v := range enum.Values {
		...
		err := &errorInfo{
			...
			HTTPCode:      enumCode,
			BizCode:       int(v.Desc.Number()),
			Comment:       comment,
			DefaultReason: strings.TrimSpace(strings.Replace(comment, "//", "", 1)),
